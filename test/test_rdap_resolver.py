...
class test_RDAP_resolver(TestCase):
    ...
    def test_raw_JSON_getter_404_on_first_try(self):
        ...
        self.assertEquals(
            "RDAP resource not found: " + self.TEST_URI, str(ex.exception)
        )
    ...
    def test_raw_JSON_getter_400_on_first_try(self):
        ...
        with self.assertRaises(RDAPResolutionException) as ex:
            self.rslvr._get_raw_RDAP_JSON(self.TEST_URI)
        self.assertEquals("RDAP request returned 400", str(ex.exception))
    ...
    def test_raw_JSON_getter_malformed_JSON_on_first_try(self):
        ...
        with self.assertRaises(RDAPResolutionException) as ex:
            self.rslvr._get_raw_RDAP_JSON(self.TEST_URI)
        self.assertEquals("Malformed JSON in RDAP output", str(ex.exception))
    ...
    def test_raw_JSON_getter_out_of_retries(self):
        ...
        with self.assertRaises(RDAPResolutionException) as ex:
            self.rslvr._get_raw_RDAP_JSON(self.TEST_URI)
        self.assertEquals(
            "Out of retries for RDAP resource: " + self.TEST_URI, str(ex.exception)
        )
    ...
    def test_resolve_from_url_success_plus_redirect(self):
        ...
        self.assertEquals(
            expected_provisional_assigned_subnet, redir_ex.exception.provisional
        )
        self.assertEquals(
            "Redirection to {0}. Provisional assignment: {1}".format(
                self.TEST_REDIR_URI, repr(expected_provisional_assigned_subnet)
            ),
            str(redir_ex.exception),
        )
    ...
    def test_resolve_from_url_dodgy_RDAP_without_whois(self):
        response = Mock(status_code=200, is_redirect=False)
        response.json = Mock(
            return_value={"startAddress": "10.0.0.0/32", "name": "foo"}
        )
        self.rslvr._session = Mock()
        self.rslvr._session.get = Mock(return_value=response)
    ...
    def test_resolve_from_url_rate_limitation_on_first_request(self, mock_sleep):
        ...
        self.assertEquals(expected_provisional_assigned_subnet, assigned_subnet)
        self.assertEquals(
            [call(self.rslvr.RATE_LIMITATION_DELAY)], mock_sleep.mock_calls
        )
    ...
    def test_resolve_from_url_rate_limitation_out_of_retries(self, mock_sleep):
        ...
        self.rslvr._session = Mock()
        self.rslvr._session.get = Mock(
            side_effect=self.rslvr.RATE_LIMITATION_RETRIES * [rate_lim_response]
        )
        ...
        self.assertEquals(
            self.rslvr.RATE_LIMITATION_RETRIES
    ...
    def test_resolve_subnet_known_RDAP_url(self):
        ...
        self._delegation_rslvr.get_top_level_assignment = Mock(
            return_value=Mock(rdap_URLs=[self.TEST_URI])
        )
        self.rslvr = RDAP_Resolver(self._delegation_rslvr)
        ...
        expected_assigned_subnet = AssignedSubnet(Address((11, 0, 0, 0)), 8, "foo")
    ...
    def test_resolve_subnet_no_RDAP_url(self):
        ...
        self.assertEquals(
            "No RDAP URL for " + repr(eleven_dot), str(no_rdap_ex.exception)
        )
