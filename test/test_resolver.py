...
from src.metadata.resolver import (DelegationResolver, RDAPRedirectException,
                                   RDAPResolutionException,
                                   ResolutionException)
...
class test_AssignedSubnetResolver(TestCase):
    ...
    def test_subnet_is_whole_address_space(self):
        ...
        self.assertRaises(
            ResolutionException,
            self.resolver.validate_assignment,
            whole_address_space,
        )

    def test_subnet_is_too_large(self):
        for prefix_len in range(1, 8):
            very_large_subnet = AssignedSubnet(
                Address("10.0.0.0"), prefix_len, "foo"
            )
            self.assertRaises(
                ResolutionException,
                self.resolver.validate_assignment,
    ...
    def test_RDAP_success_no_redirection(self):
        ...
        mock_rdap_resolver.resolve = Mock(
            return_value=eleven_dot_known_size_subnet
        )
        self.resolver._rdap_resolver = mock_rdap_resolver

        resolved_assignment = self.resolver.resolve(
            eleven_dot_unknown_size_subnet
        )
    ...
    def test_RDAP_success_but_invalid_subnet(self):
        ...
        mock_rdap_resolver = Mock()
        mock_rdap_resolver.resolve = Mock(
            return_value=eleven_dot_invalid_subnet
        )
        self.resolver._rdap_resolver = mock_rdap_resolver

        mock_whois_resolver = Mock()
        mock_whois_resolver.resolve = Mock(
            return_value=eleven_dot_valid_subnet
        )
    ...
    def test_RDAP_redirect_then_success(self):
        ...
        self.resolver._rdap_resolver.resolve_from_url = (
            mock_rdap_resolve_from_url
        )
        ...
        self.assertEquals([], mock_whois_resolver.resolve.mock_calls)

        self.assertEquals(resolved_assignment, eleven_dot_valid_subnet)

    def test_RDAP_successful_redirect_then_success(self):
        eleven_dot_unknown_size_subnet = Subnet(Address("11.12.13.0"), 32)
        eleven_dot_non_canon_subnet = Subnet(Address("11.12.13.0"), 24)
        eleven_dot_canon_subnet = Subnet(Address("11.12.13.0"), 24)
        ...
        outcomes = [
            RDAPRedirectException(
                (
                    "The princess is in another castle. Here's a "
                    "non-authoritative assignment"
                ),
                redir_url="http://other_fake_rdap/ip/11.12.13.0",
                provisional=eleven_dot_non_canon_subnet,
            ),
            eleven_dot_canon_subnet,
        ]
    ...
    def test_RDAP_successful_redirect_then_failure(self):
        ...
        outcomes = [
            ...
            RDAPResolutionException(
                "Oh no! It was one of Bowser's traps all along!"
            ),
        ]
